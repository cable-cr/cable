...
module Cable
  class Handler
    ...
    def call(context)
      ...
      Logger.info "Started GET \"#{path}\" [WebSocket] for #{remote_address} at #{Time.now.to_s}"

      context.response.headers["Sec-WebSocket-Protocol"] = "actioncable-v1-json"

      ws = HTTP::WebSocketHandler.new do |socket, context|
        Logger.info context.request.inspect
        connection = ApplicationCable::Connection.build(context.request, socket)
        ...
        socket.on_close do
          connection.close
          Logger.info "Finished \"#{path}\" [WebSocket] for #{remote_address} at #{Time.now.to_s}"
        end
      end

      Logger.info "Successfully upgraded to WebSocket (REQUEST_METHOD: GET, HTTP_CONNECTION: Upgrade, HTTP_UPGRADE: websocket)"
